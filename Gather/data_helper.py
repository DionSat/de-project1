...
def create_dataframe(df):
    ...
    df = df.astype(
        {"METERS": "int", "GPS_LONGITUDE": "float", "GPS_LATITUDE": "float"}
    )  # Set type for important columns
    df.drop(
        ["EVENT_NO_STOP", "GPS_SATELLITES", "GPS_HDOP"], axis=1, inplace=True
    )  # Drop unneeded columns for now.
    df["OPD_DATE"] = pd.to_datetime(
        df["OPD_DATE"], format="%d%b%Y:00:00:00"
    )  # Change OPD_DATE values to datetime
    ...
    df["TIME_STAMP"] = pd.to_datetime(df["OPD_DATE"], unit="s") + pd.to_timedelta(
        df["ACT_TIME"], unit="s"
    )
    return df
...
def data_assertions(df):
    ...
    df1 = pd.DataFrame()
    df1[["GPS_LONGITUDE", "GPS_LATITUDE"]] = df[["GPS_LONGITUDE", "GPS_LATITUDE"]].copy(
        deep=True
    )
    df1 = df1[
        ~df1["GPS_LONGITUDE"].isnull()
    ]  # Get all rows with non empty longitude and put them in a dataframe
    try:
        assert not df1[
            df1["GPS_LATITUDE"].isnull()
        ].values.any(), f"Some longitude doesn't have latitude"  # Check if there are any empty rows in latitude rows
    ...
    try:
        assert not df2[
            df2["OPD_DATE"].isnull() | df2["ACT_TIME"].isnull()
        ].values.any(), f"Some records don't have a date or time"  # Find any rows that have empty OPD_DATE or ACT_TIME
    except AssertionError as error:
        error_message = f"AssertionError: {error}"
        error_indices = df2[
            df2["OPD_DATE"].isnull() | df2["ACT_TIME"].isnull()
    ...
    try:
        assert not df3[
            ...
            f"Some records have actual time greater than 24 hours or less than 0"
        )
    except AssertionError as error:
        error_message = f"AssertionError: {error}"
    ...
    try:
        assert not df4[
            (df4["OPD_DATE"] <= "2022-01-01") & (df4["OPD_DATE"] >= curr)
        ].values.any(), f"Some dates are out of range"  # bool check every value in the column OPD_DATE that if they are before 2022 or after current datetime
    except AssertionError as error:
        error_message = f"AssertionError: {error}"
        error_indices = df4[
            (df4["OPD_DATE"] <= "2022-01-01") & (df4["OPD_DATE"] >= curr)
    ...
    df5["OPD_DATE"] = df5["OPD_DATE"].dt.strftime(
        "%d"
    )  # Covert OPD_DATE to datatime and then extract the day field for the rows
    try:
        assert not df5[
            df5["OPD_DATE"] != df5["OPD_DATE"][0]
        ].values.any(), f"Some dates are not on the same day"  # Check if all the values in the OPD_DATE field are the same day
    except AssertionError as error:
        error_message = f"AssertionError: {error}"
    ...
    df6 = df6[
        ~df6["GPS_LONGITUDE"].isna()
    ]  # Get all rows with non empty longitude and put them in a dataframe
    try:
        assert not df6[
            df6["GPS_LONGITUDE"].gt(0)
        ].values.any(), f"Some record has a longitude that is positive"  # bool check every value in the column GPS_LONGITUDE that is positive and check if any of those values is true
    except AssertionError as error:
        error_message = f"AssertionError: {error}"
    ...
    df7 = df7[
        ~df7["GPS_LATITUDE"].isna()
    ]  # Get all rows with non empty latitude and put them in a dataframe
    try:
        assert not df7[
            df7["GPS_LATITUDE"].lt(0)
        ].values.any(), f"Some record has a latitude that is negative"  # bool check every value in the column GPS_LATITUDE that is negative and check if any of those values is true
    except AssertionError as error:
        error_message = f"AssertionError: {error}"
    ...
    try:
        assert not df8[
            df8["METERS"].lt(0)
        ].values.any(), f"Some record have traveled a negative distance relative to total distance"  # bool check every value in the column METERS that is less than 0 meters and check if any of those values is true
    except AssertionError as error:
        error_message = f"AssertionError: {error}"
    ...
    df9 = pd.DataFrame()
    df9[["GPS_LONGITUDE", "GPS_LATITUDE"]] = df[["GPS_LONGITUDE", "GPS_LATITUDE"]].copy(
        deep=True
    )
    df9 = df9[
        ~df9["GPS_LONGITUDE"].isna()
    ]  # Get all rows with non empty longitude and put them in a dataframe
    df9 = df9[
        ~df9["GPS_LATITUDE"].isna()
    ]  # Get all rows with non empty latitude and put them in a dataframe
    try:
        assert not df9[
            df9["GPS_LONGITUDE"].lt(-180)
            | df9["GPS_LONGITUDE"].gt(180)
            | df9["GPS_LATITUDE"].gt(90)
            | df9["GPS_LATITUDE"].lt(-90)
        ].values.any(), f"Some records longitude and latitude is not on earth"
    except AssertionError as error:
        error_message = f"AssertionError: {error}"
        error_indices = df9[
            df9["GPS_LONGITUDE"].lt(-180)
    ...
    try:
        assert not df10[
            df10["SPEED"].gt(45)
        ].values.any(), f"Some records are going at unsafe high speeds"
    except AssertionError as error:
        error_message = f"AssertionError: {error}"
...
def data_splitter(df):
    breadcrumbs_df = df[
        ["EVENT_NO_TRIP", "TIME_STAMP", "GPS_LATITUDE", "GPS_LONGITUDE", "SPEED"]
    ]
    breadcrumbs_df = breadcrumbs_df.rename(
        columns={
    ...
    trip_df = trip_df.rename(
        columns={"EVENT_NO_TRIP": "trip_id", "VEHICLE_ID": "vehicle_id"}
    )
    return breadcrumbs_df, trip_df


def stop_data_process(df):
    df = df[["trip_id", "route_number", "vehicle_number", "service_key", "direction"]]
...
def insert_db(breadcrumbs_df, trip_df):
    ...
    conn = psycopg2.connect(
        host="localhost", user="postgres", password="breadcrumbs", database="postgres"
    )
    ...
    breadcrumbs_df.to_sql(
        "BreadCrumbs", con=conn_engine, if_exists="append", index=False
    )
...
def stop_insert_db(stop_df):
    ...
    # Create Stop table for stop events
    stop_df.to_sql("Stop", con=conn_engine, if_exists="replace", index=False)
...
def create_db(breadcrumbs_df, trip_df):
    ...
    breadcrumbs_df.to_sql("BreadCrumbs", con=conn, if_exists="replace", index=False)
    # Create new Trip table
    trip_df.to_sql("Trip", con=conn, if_exists="replace", index=False)
    ...
    cursor.execute(
        'ALTER TABLE "BreadCrumbs" ADD CONSTRAINT "FK_trip" FOREIGN KEY("trip_id") REFERENCES "Trip"("trip_id");'
    )
...
def create_stop_db(stop_df):
    ...
    stop_df.to_sql("Stop", con=conn, if_exists="replace", index=False)
    ...
    # Add Constraints
    cursor.execute(
        'ALTER TABLE "Stop" ADD CONSTRAINT "FK_trip" FOREIGN KEY("trip_id") REFERENCES "Trip"("trip_id");'
    )
...
def check_tables():
    ...
    # Check if BreadCrumbs table exists then get the row count
    flag = insp.has_table("BreadCrumbs", schema="public") and insp.has_table(
        "Trip", schema="public"
    )
